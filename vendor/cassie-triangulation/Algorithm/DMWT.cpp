...
void DMWT::genTriCandidates() {
  ...
  if (delaunay.compute(points, numofpoints)) {
...
void DMWT::getResultAsMatrices(Eigen::MatrixXd& V, Eigen::MatrixXi& F) {
  V.resize(numofpoints, 3);
  for (int i = 0; i < numofpoints; i++)
    for (int j = 0; j < 3; ++j)
      V(i, j) = points[3 * i + j];

  // std::cout << "#F = " << numoftilingtris << std::endl;
  F.resize(numoftilingtris, 3);
  for (int i = 0; i < numoftilingtris; i++)
    for (int j = 0; j < 3; ++j)
      F(i, j) = tris[tiling[i] * 3 + j];
}

void DMWT::getResultAsMatrices(Eigen::MatrixXd& V, Eigen::MatrixXi& F, Eigen::MatrixXd& N) {

  getResultAsMatrices(V, F);

  N.setZero(numofpoints, 3);
  if (withNormal) {
    for (int i = 0; i < numofpoints; i++)
      for (int j = 0; j < 3; ++j)
        N(i, j) = normals[3 * i + j];
